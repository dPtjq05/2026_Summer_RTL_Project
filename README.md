...
Git/Vivado 디렉터리 정형화, .gitignore 최적화, 보레이트 분주 법칙 분석
...
16배수 오버샘플링 클럭 분주기 설계 및 Testbench 시뮬레이션 검증
...
시작 비트 오버샘플링 검증 및 FSM 기반 수신기 설계
...
프레임 빌더 기반 송신기 설계 및 Loopback 통합 시뮬레이션
...
주소 디코더 및 레지스터 읽기/쓰기 버스 인터페이스 통합 검증
...
목표 보레이트 (Baud Rate): $9600\text{ bps}$

오버샘플링 배수: $16\text{x}$ (노이즈 필터링 및 안정적인 비트 센터 저격을 위함)

목표 틱 주파수 ($f_{\text{tick}}$): $9600 \times 16 = 153,600\text{ Hz}$
...
Vivado를 실행하고 vivado_prj/2026_Summer_RTL_Project.xpr을 오픈합니다.

소스 경로가 src/ 및 tb/ 폴더 내의 파일로 정상 링크되어 있는지 확인합니다.